...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    $id = $_GET['id'];
    $stmt = mysqli_prepare($con, "SELECT * FROM `employee` WHERE Id = ? LIMIT 1");
        ...
        <div class="flex flex-col text-[20px]">
            <form method="post">
                <div class="py-[20px]">
                    ...
                    <input type="text" name="txtName" value="<?php echo isset($row['Name']) ? $row['Name'] : '' ?>" class="w-[500px] h-[40px] text-[18px] pl-[10px] border-[2px] border-[#cccaca]">
                </div>

                <div class="py-[20px]">
                    <label class="pr-[20px]">Charge</label>
                    <input type="number" name="txtCharge" value="<?php echo isset($row['Charge']) ? $row['Charge'] : '' ?>" class="w-[500px] h-[40px] text-[18px] px-[10px] border-[2px] border-[#cccaca]">
                </div>
